...
import { getUserProfile, getAllAppHeader, deleteAppHeader, getAppHeaderById } from '../../util/APIUtils';
...
class Account extends Component {
	...
	componentDidMount() {
		...
		getAllAppHeader()
			.then((datas) => {
				const AppHeader = []
				datas.map((dt, key) => {
					let data_cache = {id: key + 1, key: dt.id, name: dt.name, description: dt.description, uploaded: dt.publish_date}
	...
	editAppHeader = (id) => {
		getAppHeaderById(id)
			.then((response) => {
				this.props.router.push({pathname: '/datasets-submit', state: response});
			})
			.catch((error) => {
				notification.error({
					message: 'Observatory Polinema',
												...
												<div class="p-5">
													<div class="text-center">
														...
														<h1 class="h6 text-gray-600 mb-4">
															Timejoined - {this.state.user.joinedAt}
														</h1>
